...
function onHeaderName(parser, from, size) {
	parser.context[lastHeaderName] = extractPayload(parser, from, size).toString(
		"utf-8",
	);
	return 0;
}
...
function onHeaderValue(parser, from, size) {
	parser.context[parser.context[lastHeaderName]] = extractPayload(
		parser,
		from,
		size,
	).toString("utf-8");
...
function onHeaders(parser) {
	delete parser.context[lastHeaderName];
	return 0;
}
