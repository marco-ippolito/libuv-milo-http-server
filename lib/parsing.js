...
function onHeaderName(_, from, size) {
  this.parsed[lastHeaderName] = extractPayload(this, from, size).toString("utf-8");
}

function onHeaderValue(_, from, size) {
  this.parsed.headers[this.parsed[lastHeaderName]] = extractPayload(this, from, size).toString("utf-8");
}
