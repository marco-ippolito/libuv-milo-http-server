...
function getValue(parser, name, from, size) {
	parser.context.parsed[name] = extractPayload(parser, from, size).toString("utf-8");
	return 0;
}
...
function onHeaderName(parser, from, size) {
	parser.context.parsed.headers[lastHeaderName] = extractPayload(parser, from, size).toString(
		"utf-8",
	);
	return 0;
}
...
function onHeaderValue(parser, from, size) {
	parser.context.parsed.headers[parser.context.parsed.headers[lastHeaderName]] = extractPayload(
		parser,
		from,
		size,
	).toString("utf-8");
